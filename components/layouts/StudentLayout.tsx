...
const StudentLayout: React.FC<StudentLayoutProps> = ({ children, activeRoute, onNavigate, onLogout }) => {
    ...
    return (
        <div className="flex h-screen bg-indigo-50/30 text-slate-800 overflow-hidden font-sans selection:bg-indigo-200">
            {/* Mobile Header */}
            ...
            {/* Gamified Sidebar */}
            <aside className={`
                fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white/90 lg:bg-white/70 backdrop-blur-xl border-r border-white/50 flex flex-col transition-transform duration-300 shadow-xl lg:shadow-sm
            ...
            `}>
                <div className="p-6 border-b border-indigo-100/50 bg-gradient-to-b from-indigo-50/50 to-transparent mt-16 lg:mt-0">
                    <div className="flex items-center gap-3 mb-4">
                        <div className="w-12 h-12 rounded-full bg-indigo-100 border-2 border-indigo-200 flex items-center justify-center text-lg text-indigo-600 shadow-sm">
                            👨‍🎓
                        </div>
                        <div>
                            <h3 className="font-bold text-sm text-slate-800">Alex Student</h3>
                            <span className="text-[10px] text-indigo-500 uppercase tracking-widest font-bold">Level 12 Scholar</span>
                        </div>
                    </div>
                    <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden border border-slate-100">
                        <div className="bg-gradient-to-r from-indigo-400 to-purple-400 h-full w-[75%] shadow-sm"></div>
                    </div>
            ...
            {/* Main Content Area */}
            <main className="flex-1 relative overflow-hidden bg-[#f8fafc] w-full flex flex-col">
                {/* Desktop Header */}
                ...
                <div className="flex-1 overflow-y-auto relative z-10 scroll-smooth p-6 lg:p-10 mt-16 lg:mt-0">
